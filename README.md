...
- ink.js：ページ全体の背景のインク水彩にじみ演出（WebGL / 依存ライブラリなし）
- blue_water_lily_edit_large.jpg：プロフィール画像（画質そのまま）
- blue_water_lily_fabicon.png：ファビコン画像（画質低め）
- CNAME：独自ドメイン設定用

### 主な特徴
- 軽量構造：HTML + CSS + 最小限のJavaScriptで構成（JSは背景の演出のみ・依存ライブラリなし）
...
- 背景はページ全体でインクが水彩のように溶け合う演出（`ink.js`）
  - 素のWebGLフラグメントシェーダー（fbmノイズ＋ドメインワープ）で実装
  - 本文（`main` / `footer`）は半透明のすりガラス下地（`backdrop-filter`）で可読性を確保
  - `prefers-reduced-motion` 指定時・WebGL非対応時は静的グラデーションにフォールバック
  - 低解像度レンダリング＋30fps上限＋タブ非表示で描画停止（モバイルの電池・発熱対策）
